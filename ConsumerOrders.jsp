	...
	<div class="wrapper d-flex align-items-stretch">
		<nav id="sidebar">
			...
			<ul class="list-unstyled components mb-5">
				...
				<li class="active"><a href="ConsumerOrders.jsp"><span
			...
			<div id="results" class="container">
				<!--  -->
					...
					int f=0;
						...
						Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/farmerzone", "Bootathon",
						"6383411123");
						PreparedStatement st = (PreparedStatement) con.prepareStatement("select * from payment where consumernumber=?");
						st.setString(1, number);
						ResultSet r1 = st.executeQuery();
						...
						while (r1.next()) {
							...
							out.println("<p class='card-title'>Product : " + r1.getString("Quantity") + "kg</p>");
							out.println("<p class='card-title'>Descritption : " + r1.getString("Description") + "</p>");
							if (r1.getString("status").equals("waiting")) {
						out.println("<p style='color:orange' class='card-title'><b>Waiting to accept or cancel</b></p>");
						out.println("<form action='ordercancel' method='POST'><input type='hidden' name='farmernum' value='"
								+ r1.getString("Farmernumber") + "'><input type='hidden' name='consnum' value='"
								+ r1.getString("consumernumber") + "'><input type='hidden' name='prod' value='"
								+ r1.getString("Productname")
								+ "'><b>CLick to cancel </b><button type='submit' class='btn btn-success' style='border-radius: 50%'><i class='fa fa-ban'></i></button></form>");
						out.println("<br>");
						...
						out.println("<p style='color:Red' class='card-title'><b>Canceled</b></p>");
							}
							//out.println("<td>");
						...
						if (f == 0) {
							...
							out.println("<h1 style='text-align:center;color:red'>No Orders Placed</h1>");
							out.println("<h6 style='text-align:center;color:red'>Place some order</h6>");
						}
					%>
			</div>
		</div>
