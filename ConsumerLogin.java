...
public class ConsumerLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PreparedStatement st=(PreparedStatement) con.prepareStatement("select Phonenumber,Password from Consumerregister where `Phonenumber` = ? and `Password`=?");
			String dbuser="";
			String dbpass="";
			st.setString(1,number);
			 ...
			 if(dbuser.equals(number) && dbpass.equals(password))
			 {
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/ConsumerDashboard.jsp");
